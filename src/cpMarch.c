...
cpMarchCells(
  ...
  cpMarchSampleFunc sample, void *sample_data,
	...
	cpFloat x_denom = fix14_inverse((cpFloat)(x_samples - 1));
	cpFloat y_denom = fix14_inverse((cpFloat)(y_samples - 1));
	...
	for(unsigned long i=0; i<x_samples; i++) buffer[i] = sample(cpv(cpflerp(bb.l, bb.r, fix14_mul(i, x_denom)), bb.b), sample_data);
	
	for(unsigned long j=0; j<y_samples-1; j++){
		cpFloat y0 = cpflerp(bb.b, bb.t, fix14_mul((j+0), y_denom));
		cpFloat y1 = cpflerp(bb.b, bb.t, fix14_mul((j+1), y_denom));
		...
		for(unsigned long i=0; i<x_samples-1; i++){
			cpFloat x0 = cpflerp(bb.l, bb.r, fix14_mul((i+0), x_denom));
			cpFloat x1 = cpflerp(bb.l, bb.r, fix14_mul((i+1), x_denom));
